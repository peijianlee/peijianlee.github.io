...
var app = new Vue({
	...
	methods: {
		...
		matchHome: function matchHome(h, key, i) {
			console.log(h + ',' + key + ',' + i);
			// if(!i) {
			// 	return '<span>' + h + '</span>'
			// } else if (key === 'cover') {
			...
			// 	// console.log(h)
			// 	var x,
			// 		spanHTML = '<span>_ ' + key + '：</span><div class="home-skill">'
			// 	for(x in h){
			// 		spanHTML += '<span>' + h[x] + '</span>'
			// 	}
			// 	return spanHTML + '</div>'
			// } else {
			// 	return '<span>_ ' + key + '：</span>' + '<span>' + h + '</span>'
			// }
		}
	},
	watch: {
		pageIndex: function pageIndex(n) {
			// var $Userinfo = this.$refs.Userinfo
...
var lpj = exports.lpj = {
	...
	"Skills": [{
		...
		"skill": "CSS/CSS3",
		"des": "比较熟练使用 CSS 进行对网页的排版，能使用 css3 animation 属性对标签添加简单的动画。对Sass、Less、Stylus 的预编译也有一定的理解。"
	}, {
		"icon": {
