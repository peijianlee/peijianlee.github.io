...
var app = new Vue({
	...
	mounted: function mounted() {
		...
		var timer = setInterval(function () {
			_this.val = _this.val + step;
			_this.$refs.progress.style.width = _this.val + '%';

			if (_this.val >= _this.stopVal) {
				clearInterval(timer);
				return;
			}
		}, 10);
	...
	watch: {
		isOk: function isOk() {
			...
			var timer = setInterval(function () {
				val = val + step;
				_this2.$refs.progress.style.width = 100 + '%';
				if (val >= 100) {
					clearInterval(timer);
					_this2.isShow = false;

					console.log(_this2.isShow);
					_this2.$emit('callback', 'load success');
					return;
				}
			}, 10);
		}
